...
SESSION_TIMEOUT = tools.config.get('redis_expiration', 604800)


def is_redis_session_store_activated():
    return tools.config.get('enable_redis', False)
...
try:
    ...
    if is_redis_session_store_activated():
        raise ImportError('Please install package with [pip3 install redis]')
